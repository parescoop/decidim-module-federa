  ...
  module Federa
    module Utils
      ...
      def metadata_xml(overrides = {})
        metadata = Decidim::Federa::Metadata.new
        metadata.to_xml(Decidim::Federa::Settings::Metadata.new(overrides))
      end

      def sso_saml(sso_params, options)
        request = Decidim::Federa::SsoRequest.new(sso_params, options)
        session[:"tenant-federa-name"] = options["name"]
      ...
      def slo_saml(slo_params, options)
        logout_request = Decidim::Federa::SloRequest.new(slo_params, options)
        session[:"#{session_prefix}slo_id"] = logout_request.uuid
        logout_request.to_saml
      end

      def sso_request(saml_response, options)
        r = Decidim::Federa::SsoResponse.new(saml_response, session[:"#{session_prefix}sso_params"], options)
        response = r.response
        valid = r.valid?
        if valid
          ...
          msg = 'decidim.federa.sso_request.success'
        else
          matches = nil
          if r.errors && r.errors.any?{ |a| matches = a.match(/The status code of the Response was not Success, was Responder => AuthnFailed -> ErrorCode nr(19|2[0-5])/) } && (error_code = matches.try(:[], 1)).present?
            msg = "decidim.federa.sso_request.failure_#{error_code}"
          else
            msg = 'decidim.federa.sso_request.failure'
          end
        end
        [valid, msg, response]
      end
      ...
      def slo_request(saml_response, options)
        response = Decidim::Federa::SloResponse.new(saml_response, session[:"#{session_prefix}sso_params"].merge(slo_id: session[:"#{session_prefix}slo_id"]), options)
        ...
        if valid
          ...
          msg = 'decidim.federa.slo_request.success'
        else
          msg = 'decidim.federa.slo_request.failure'
        end
        [valid, msg]
      end
      ...
      def slo_params
        ...
        sso_params[:federa_level] = session[:"#{session_prefix}sso_params"]["federa_level"]
